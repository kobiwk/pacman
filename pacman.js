( function() {
	...
	function resultOfGame(obj) {
		...
		if ( x <= (that.x + 26) && that.x <= ( x + 26 ) && y <= ( that.y + 26 ) && that.y <= ( y + 26) ) {
			//... takes Pacman lives
			pacmanLives--;
			...
			context.fillText("Pacman: " + pacmanLives + " Score: " + score, 10, 30);
			context.fillStyle = "blue";
			detectCollison(new DefinePacman, obj);
	...
	function detectCollison(pacman, enemy) {
		if ( x <= (enemy.x + 26) && enemy.x <= ( x + 26 ) && y <= ( enemy.y + 26 ) && enemy.y <= ( y + 26 ) ) {
	...
	DefinePacman.prototype = {
		...
		move: function() {
			...
			if ( openMouth % 2 ===  0 ) {
				if ( direction === "right" ) {
					context.drawImage(loadBeignsImages, 320, 0, 32, 32, x, y, 32, 32);
				} else if ( direction === "left" ) {
					context.drawImage(loadBeignsImages, 352, 64, 32, 32, x, y, 32, 32);
				} else if ( direction === "down" ) {
					context.drawImage(loadBeignsImages, 320, 32, 32, 32, x, y, 32, 32);
				} else if ( direction === "up" ) {
					context.drawImage(loadBeignsImages, 320, 96, 32, 32, x, y, 32, 32);
				}
				
			} else if ( openMouth % 2 ===  1 ) {
				if ( direction === "right" ) {
				context.drawImage(loadBeignsImages, 352, 0, 32, 32, x, y, 32, 32);
				} else if ( direction === "left" ) {
					context.drawImage(loadBeignsImages, 320, 64, 32, 32, x, y, 32, 32);
				} else if ( direction === "down" ) {
					context.drawImage(loadBeignsImages, 352, 32, 32, 32, x, y, 32, 32);
				} else if ( direction === "up" ) {
					context.drawImage(loadBeignsImages, 352, 96, 32, 32, x, y, 32, 32);
				}
			}
